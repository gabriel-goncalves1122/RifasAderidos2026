...
export function CheckoutModal({
  ...
  useEffect(() => {
    if (!open) {
      ...
      setEmail("");
      setComprovante(null);
      setShowSuccess(false); // Reseta a tela de sucesso ao fechar
    }
  }, [open]);
  ...
  const handleCopiarPix = () => {
    ...
    // TODO: Trocar para Snackbar depois
  ...
  const handleFinalizar = async () => {
    ...
    if (sucesso) {
      // Dispara a animação visual de sucesso
      setShowSuccess(true);

      // Aguarda 3 segundos para o usuário ler a mensagem, depois fecha o modal e limpa o carrinho
      setTimeout(() => {
        onSuccess();
      }, 3000);
    }
  ...
  return (
    <Dialog
      open={open}
      // Trava o fechamento se estiver carregando ou exibindo a mensagem de sucesso
      onClose={loading || showSuccess ? undefined : onClose}
      fullWidth
      maxWidth="sm"
    >
      {/* TELA DE SUCESSO (Rendeziada condicionalmente) */}
      {showSuccess ? (
        <Fade in={showSuccess}>
          <Box
            sx={{
